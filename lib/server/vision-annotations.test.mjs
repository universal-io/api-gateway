...
// app-ios measured this sentence as worth 0.155 of vertical error — every box
// in the right column and the wrong row, with the wording still correct
// (lessons-from-app-mac.md §3-b). It is the single highest-value line in the
// prompt and the easiest to lose in a refactor, because nothing fails without it.
test("the image size is stated when coordinates are requested", () => {
  const prompt = buildVisionPromptText({
    ...BASE,
    wantsAnnotations: true,
    imageSize: { width: 1536, height: 864 },
  });
  assert.ok(prompt.includes("exactly 1536 pixels wide and 864 pixels tall"));
  assert.ok(prompt.includes("x and w by 1536"));
  assert.ok(prompt.includes("y and h by 864"));
});

// Shipped clients resolve measured frames through candidate ids and never draw
// from these numbers, so their prompt stays exactly as it was.
test("the image size is left out when no coordinates were asked for", () => {
  const prompt = buildVisionPromptText({ ...BASE, imageSize: { width: 1536, height: 864 } });
  assert.ok(!prompt.includes("pixels wide"));
});
