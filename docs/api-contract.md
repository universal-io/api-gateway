...
- `input.pointer`: 任意。**AXを持たないクライアント（web）が「指した場所」を送る**。
  ...
  **🔴 座標の精度について（app-ios が実測で支払った授業料。再履修しないこと）**
  正本は `app-ios/docs/lessons-from-app-mac.md` と `app-ios/docs/investigation-highlight-offset.md`。
  - **画像サイズは必ずモデルに伝える。** 伝えないとモデルは想定した高さ（約1080px）を
    分母にY座標を正規化し、**解説文は完全に正しいまま枠だけが別の行に出る**。
    実測: 縦誤差 0.155 → 0.001。Gatewayは `lib/server/image-size.ts` で
    **画像バイトから導出**する（クライアントの申告は信用しない。送り忘れと不一致を
    起こしえなくするため）
  - **長辺は1536以下に縮小して送る。** 実寸の写真は命中3/11まで落ちる。
    「縮小すると読み違える」は実測で否定された（1/6でもバージョン番号を正確に読む）
  - **誤差はバイモーダル。** 当てる（0.01前後）か大きく外す（0.4前後）かで中間がない。
    平均を見て定数オフセットで補正すると、当たっている回を壊す
  - **最終的な解は実測への接地**（`candidates`）。モデルの目測は入力条件が変わると壊れる

  **🔴 クライアントは可能なら、指した場所を画像そのものに描き込んでから送ること**
  （point=十字入りのリング、region=矩形。マゼンタ＋白フチ）。
  座標を数値で伝えるだけだとモデルが位置を取り違える。2026-08-16のweb版初回実測では、
  画面中央のボタンをタップしたのに右上の閉じるボタンについて回答した。
  **同じモデルが自分で選んだ対象には正確な枠を返す**ので、画像から座標を読むことは
  できている。できないのは数値から位置を割り出すこと。promptは
  「マークが見えるならそちらを座標より信じよ」と指示するため、描かないクライアントでも
  従来どおり動作する（劣化するだけで壊れない）。
